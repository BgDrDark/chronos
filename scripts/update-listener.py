...
def _run_deploy(project_dir, version, api_key):
...
class DeployHandler(BaseHTTPRequestHandler):
    """HTTP handler for deploy requests"""
    ...
    def _verify_api_key(self):
        """Verify DeployKey header"""
        auth = self.headers.get("Authorization", "")
        if auth.startswith("DeployKey "):
            provided_key = auth[10:]
            if self.api_key and hmac.compare_digest(provided_key, self.api_key):
                return True
        return False
        ...
        if self.path == "/deploy":
            ...
            with _deploy_lock:
                if _deploy_status["is_deploying"]:
                    self._send_json({"error": "Deploy already in progress"}, 409)
                    return
            ...
            thread = threading.Thread(
                target=_run_deploy,
                args=(self.project_dir, version, self.api_key),
                daemon=True
            )
            thread.start()
            ...
            self._send_json({
                "status": "started",
                "message": f"Deployment started{' to ' + version if version else ''}",
                "version": version,
...
def main():
    parser = argparse.ArgumentParser(description="Chronos Deploy Listener")
    ...
    parser.add_argument("--api-key", type=str, default=None, help="Deploy API key for authentication")
