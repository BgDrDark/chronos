...
class EnhancedPayrollCalculator(PayrollCalculator):
    ...
    def __init__(self, db_session: AsyncSession, company_id: int, user_id: int, calculation_period: dict[str, date], preloaded_data: dict[str, Any] | None = None):
    ...
    async def calculate_enhanced_payroll(self) -> dict[str, Any]:
        ...
        await self._calculate_noi_payments()
    ...
    async def _load_additional_components(self) -> None:
        ...
        adv_query = select(AdvancePayment).where(
            and_(
                ...
                not AdvancePayment.is_processed,
            ),
        )
    ...
    async def _get_base_salary(self) -> Decimal | None:
    ...
    async def _get_daily_gross_salary(self) -> Decimal | None:
    ...
    async def _calculate_gross_net(self, base_salary: Decimal | None, base_calculation: dict[str, Any]) -> tuple[Decimal, Decimal]:
    ...
    async def get_contract_segments_for_period(
        ...
        for annex in annexes:
            if annex.effective_date > period_start:
                ...
                new_salary = annex.base_salary or (
