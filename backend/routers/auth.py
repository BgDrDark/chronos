...
from backend.auth.qr_utils import generate_qr_secret, get_dynamic_qr_token
...
from backend.database.models import KioskDevice
...
async def get_my_qr_token(
    ...
    stmt = select(KioskDevice).where(KioskDevice.is_active)
...
async def upload_avatar(
    ...
    if db_user.profile_picture:
        ...
        if os.path.exists(old_path):
            with contextlib.suppress(BaseException): os.remove(old_path)
