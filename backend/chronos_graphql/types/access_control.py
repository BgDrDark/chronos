...
class AccessZone:
    ...
    @strawberry.field
    async def parent_zone(self, info: strawberry.Info) -> Annotated["AccessZone", strawberry.lazy("backend.chronos_graphql.types.access_control")] | None:  # noqa: F821
    ...
    @strawberry.field
    async def children(self, info: strawberry.Info) -> list[Annotated["AccessZone", strawberry.lazy("backend.chronos_graphql.types.access_control")]]:  # noqa: F821
        from sqlalchemy import select
        db = info.context["db"]
    ...
    @strawberry.field
    async def authorized_users(self, info: strawberry.Info) -> list[Annotated["User", strawberry.lazy("backend.chronos_graphql.types.user")]]:  # noqa: F821
...
class AccessDoor:
    ...
    @strawberry.field
    async def gateway(self, info: strawberry.Info) -> Annotated["Gateway", strawberry.lazy("backend.chronos_graphql.types.hardware")] | None:  # noqa: F821
        from backend.chronos_graphql.types.hardware import Gateway
        db = info.context["db"]
