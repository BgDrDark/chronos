...
from backend.chronos_graphql.types import Company, Department, Position, Role

if TYPE_CHECKING:
    from backend.chronos_graphql.types.contract import EmploymentContract
...
class User:
    ...
    @strawberry.field
    async def payrolls(self, info: strawberry.Info) -> list[Annotated["Payroll", strawberry.lazy("backend.chronos_graphql.types.payroll")]]:  # noqa: F821
    ...
    @strawberry.field
    async def employment_contract(self, info: strawberry.Info) -> Annotated["EmploymentContract", strawberry.lazy("backend.chronos_graphql.types.contract")] | None:
        from backend.chronos_graphql.types.contract import EmploymentContract
    ...
    @strawberry.field
    async def active_contract(self, info: strawberry.Info) -> Annotated["EmploymentContract", strawberry.lazy("backend.chronos_graphql.types.contract")] | None:
        from backend.chronos_graphql.types.contract import EmploymentContract
        result = await info.context["dataloaders"]["contract_by_user_id"].load(self.id)
        return EmploymentContract.from_pydantic(result) if result else None

    @strawberry.field
    async def bonuses(self, info: strawberry.Info) -> list[Annotated["Bonus", strawberry.lazy("backend.chronos_graphql.types.payroll")]]:  # noqa: F821
        from backend.chronos_graphql.types.payroll import Bonus
        db = info.context["db"]
        from sqlalchemy import select
    ...
    @strawberry.field
    async def leave_balance(self, info: strawberry.Info) -> Annotated["LeaveBalance", strawberry.lazy("backend.chronos_graphql.types.leave")] | None:  # noqa: F821
        from backend import schemas
        from backend.crud.repositories import time_repo
        from backend.chronos_graphql.types.leave import LeaveBalance
        db = info.context["db"]
    ...
    ) -> list[Annotated["TimeLog", strawberry.lazy("backend.chronos_graphql.types.time_tracking")]]:  # noqa: F821
        from backend import schemas
        from backend.chronos_graphql.types.time_tracking import TimeLog
        db = info.context["db"]
        current_user = info.context["current_user"]
