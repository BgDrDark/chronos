...
from backend.database.models import User, Role, TimeLog, Payroll, Payslip, Shift, WorkSchedule, GlobalSetting, LeaveRequest, LeaveBalance, sofia_now, AuthKey, UserSession, AuditLog, ShiftSwapRequest, ScheduleTemplate, ScheduleTemplateItem, PushSubscription, UserDocument, APIKey, Webhook, WorkplaceLocation
...
async def create_user(db: AsyncSession, user: schemas.UserCreate, role_name: str = "user"):
...
# --- Time Logs ---
async def create_time_log(db: AsyncSession, log_data: schemas.TimeLogCreate):
    ...
    await log_audit_action(
        db, log_data.user_id, "time_log_created",
        details=json.dumps({"log_id": db_log.id, "start_time": log_data.start_time.isoformat()})
    )
    
    return db_log

async def get_time_logs_by_user(db: AsyncSession, user_id: int, start_date: date = None, end_date: date = None):
    """Get time logs for a user"""
    stmt = select(TimeLog).where(TimeLog.user_id == user_id)
    
    if start_date:
        stmt = stmt.where(func.date(TimeLog.start_time) >= start_date)
    if end_date:
        stmt = stmt.where(func.date(TimeLog.start_time) <= end_date)
    
    stmt = stmt.order_by(desc(TimeLog.start_time))
    result = await db.execute(stmt)
    return result.scalars().all()


# --- Payroll Config ---
async def create_payroll_config(db: AsyncSession, config_data: schemas.PayrollConfigCreate):
    """Create individual payroll configuration for a user"""
    db_config = Payroll(
        user_id=config_data.user_id,
        hourly_rate=float(config_data.hourly_rate),
        overtime_multiplier=float(config_data.overtime_multiplier_150),
        standard_hours_per_day=config_data.working_hours_per_day,
        monthly_salary=float(config_data.base_salary),
        annual_leave_days=20,
        tax_percent=10.0,
        health_insurance_percent=3.2,
...
async def get_payroll_config(db: AsyncSession, user_id: int):
    """Get payroll configuration for user"""
    # Try to get individual config
...
async def get_global_payroll_config(db: AsyncSession):
    ...
    # Create a mock Payroll object for global config
    return Payroll(
        hourly_rate=10.0,
        overtime_multiplier=1.5,
        standard_hours_per_day=8,
        monthly_salary=1600.0,
        annual_leave_days=20,
        tax_percent=10.0,
        health_insurance_percent=13.78,
        has_tax_deduction=True,
        has_health_insurance=True,
        currency="EUR"
    )
...
# --- Payroll Records ---
async def create_payroll_record(db: AsyncSession, record_data: schemas.PayrollRecordCreate):
    """Create a payroll record"""
    db_record = Payroll(
        user_id=record_data.user_id,
        gross_salary=float(record_data.gross_salary),
        net_salary=float(record_data.net_salary),
        taxes=float(record_data.taxes),
        social_security=float(record_data.social_security),
        health_insurance=float(record_data.health_insurance),
    ...
    await log_audit_action(
        db, record_data.user_id, "payroll_record_created",
        details=json.dumps({"record_id": db_record.id, "gross_salary": float(record_data.gross_salary)})
    )
...
async def calculate_monthly_payroll(db: AsyncSession, user_id: int, year: int, month: int):
    """Calculate monthly payroll for a user"""
    # Get payroll config
    config = await get_payroll_config(db, user_id)
    if not config:
        raise HTTPException(status_code=404, detail="Payroll configuration not found")
    
    # Get time logs for month
    ...
    for log in time_logs:
        ...
        if log.start_time.weekday() >= 5:  # Weekend
            weekend_hours += duration
        else:  # Weekday
            if normal_hours < 160:  # Regular hours limit (8 * 20 days)
                remaining_regular = 160 - normal_hours
                if duration <= remaining_regular:
                    normal_hours += duration
                else:
                    normal_hours += remaining_regular
                    overtime_hours += duration - remaining_regular
            else:
    ...
    overtime_pay = overtime_hours * hourly_rate * config.overtime_multiplier
    weekend_pay = weekend_hours * hourly_rate * 2.0  # 200% for weekends
    
    gross_salary = float(config.monthly_salary) + overtime_pay + weekend_pay
    
    # Calculate deductions (simplified)
    taxes = gross_salary * 0.10
    social_security = gross_salary * 0.098
    health_insurance = gross_salary * 0.032
    total_deductions = taxes + social_security + health_insurance
    net_salary = gross_salary - total_deductions
    
    return schemas.PayrollCalculation(
        user_id=user_id,
        ...
        weekend_holiday_pay=Decimal(str(weekend_pay)),
        bonus=Decimal('0.00'),
        deductions=Decimal(str(total_deductions))
    )
...
# --- Audit Logs ---
async def get_audit_logs(db: AsyncSession, user_id: int = None):
    """Get audit logs"""
...
from backend.database.models import User, Role, TimeLog, Payroll, Payslip, Shift, WorkSchedule, GlobalSetting, \
    ...
    ScheduleTemplateItem, PushSubscription
...
async def update_swap_request_status(
    ...
    if not swap:
        raise ValueError("Заявката за размяна не е намерена.")

    valid_statuses = ["pending", "accepted", "rejected", "approved"]
    if new_status not in valid_statuses:
        raise ValueError("Невалиден статус за заявката.")
    ...
    if new_status == "approved":
        ...
        await create_notification(db, swap.requestor_id,
                                  f"Вашата размяна на смени за {req_sched.date} беше ОДОБРЕНА от администратор.")
        await create_notification(db, swap.target_user_id,
                                  f"Вашата размяна на смени за {tar_sched.date} беше ОДОБРЕНА от администратор.")
    ...
    try:
        ...
        if not user:
            if include_failed_logins:
                await log_audit_action(
                    db, None, "LOGIN_FAILED_EMAIL_NOT_FOUND",
        ...
        if not user.is_active:
            if include_failed_logins:
                await log_audit_action(
                    db, user.id, "LOGIN_FAILED_INACTIVE",
                    details=f"Email: {email}, User is inactive"
                )
            return None
        ...
        if not verify_password(password, user.hashed_password):
            if include_failed_logins:
                await log_audit_action(
                    db, user.id, "LOGIN_FAILED_WRONG_PASSWORD",
                    details=f"Email: {email}, Wrong password"
                )
            return None
...
async def log_audit_action(db: AsyncSession, user_id: int, action: str, target_type: str, target_id: int, details: str):
    log_entry = AuditLog(
        user_id=user_id,
        action=action,
        target_type=target_type,
...
async def update_shift(db: AsyncSession, shift_id: int, name: str = None, start_time: datetime.time = None,
                       end_time: datetime.time = None, tolerance_minutes: int = None,
...
async def get_all_users(db: AsyncSession, skip: int = 0, limit: int = 100, search: Optional[str] = None,
                        sort_by: str = "id", sort_order: str = "asc"):
...
async def get_timelogs_by_date_range(
        db: AsyncSession,
        user_id: int,
        start_date: date,
        end_date: date,
    ...
    """Get time logs for a user within a date range with optional shift information"""
    try:
        ...
        if include_shifts:
            stmt = stmt.options(selectinload(TimeLog.user))

        stmt = stmt.order_by(desc(TimeLog.start_time)).offset(offset).limit(limit)
        result = await db.execute(stmt)
        return result.scalars().all()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving time logs: {str(e)}"
        )


async def get_time_logs_by_user_and_date(
        db: AsyncSession,
        user_id: int,
        target_date: date
) -> List[TimeLog]:
    """Get time logs for a specific user and date"""
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving time logs for user {user_id} on {target_date}: {str(e)}"
        )
...
async def calculate_monthly_payroll(
    ...
    try:
        ...
        if not config:
            raise HTTPException(
                status_code=404,
                detail=f"Payroll configuration not found for user {user_id}"
            )
        ...
        for log in time_logs:
            ...
            if log.start_time.weekday() >= 5:  # Weekend
                ...
                monthly_standard = config.standard_hours_per_day * 22  # Average work days
                if normal_hours < monthly_standard:
                    remaining_regular = monthly_standard - normal_hours
                    if duration <= remaining_regular:
                        normal_hours += duration
                    else:
                        normal_hours += remaining_regular
        ...
        if config.has_tax_deduction:
            tax_amount = gross_salary * (config.tax_percent / 100)

        if config.has_health_insurance:
            insurance_amount = gross_salary * (config.health_insurance_percent / 100)

        total_deductions = tax_amount + insurance_amount
        net_salary = gross_salary - total_deductions
        ...
        payslip_data = {
            ...
            "total_regular_hours": normal_hours,
            "total_overtime_hours": overtime_hours,
            "regular_amount": normal_pay,
            "overtime_amount": overtime_pay + weekend_pay,
            "bonus_amount": 0.0,
        ...
        return await create_payslip(db, payslip_data)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error calculating payroll for user {user_id}: {str(e)}"
        )
...
async def generate_payslip(
    ...
    """Generate a payslip for a specific period"""
    try:
        # Validate dates
        ...
        stmt = select(TimeLog).where(
            TimeLog.user_id == user_id,
            TimeLog.start_time >= period_start,
            TimeLog.start_time <= period_end,
            TimeLog.end_time.isnot(None)  # Only completed logs
        )
        result = await db.execute(stmt)
        time_logs = result.scalars().all()
        ...
        for log in time_logs:
            ...
            if log.start_time.weekday() >= 5:  # Weekend
                ...
                weekend_pay += duration_hours * float(config.hourly_rate) * 2.0
            else:  # Weekday
                standard_daily_hours = config.standard_hours_per_day or 8
                if regular_hours < (standard_daily_hours * 22):  # Monthly regular hours
                    remaining_regular = (standard_daily_hours * 22) - regular_hours
                    if duration_hours <= remaining_regular:
                        regular_hours += duration_hours
                        regular_pay += duration_hours * float(config.hourly_rate)
                    else:
                        regular_hours += remaining_regular
                        regular_pay += remaining_regular * float(config.hourly_rate)
                        overtime_hours += duration_hours - remaining_regular
                        overtime_pay += (duration_hours - remaining_regular) * float(
                            config.hourly_rate) * config.overtime_multiplier
                else:
                    overtime_hours += duration_hours
                    overtime_pay += duration_hours * float(config.hourly_rate) * config.overtime_multiplier

        # Get bonuses for the period
        from backend.database.models import Bonus
        bonus_stmt = select(Bonus).where(
            Bonus.user_id == user_id,
            Bonus.date >= period_start.date(),
            Bonus.date <= period_end.date()
        )
        ...
        # Calculate totals
        gross_amount = float(config.monthly_salary or 0) + regular_pay + overtime_pay + weekend_pay + total_bonus
        tax_amount = gross_amount * (config.tax_percent / 100) if config.has_tax_deduction else 0
        insurance_amount = gross_amount * (config.health_insurance_percent / 100) if config.has_health_insurance else 0
        total_amount = gross_amount - tax_amount - insurance_amount
        ...
        # Log action
        await log_audit_action(
            db, admin_user_id or user_id, "GENERATE_PAYSLIP",
            target_type="Payslip", target_id=payslip.id,
            details=f"Generated payslip for user {user_id} for period {period_start.date()} to {period_end.date()}, Amount: {total_amount}"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error generating payslip for user {user_id}: {str(e)}"
        )
...
async def update_payroll_status(
    ...
    """Update payroll/payslip status with audit logging"""
    try:
        # Validate status
        ...
        # Get payslip
        payslip = await db.get(Payslip, record_id)
        if not payslip:
            raise HTTPException(
                status_code=404,
                detail=f"Payslip with ID {record_id} not found"
            )
        ...
        payslip.updated_at = sofia_now()

        # Add notes if provided
        if notes:
            # Assuming there's a notes field, otherwise we'll add it to details
            await log_audit_action(
                db, updated_by, f"UPDATE_PAYROLL_STATUS_{status.upper()}",
                target_type="Payslip", target_id=record_id,
                details=f"Changed status from {old_status} to {status}. Notes: {notes}"
            )
        else:
            await log_audit_action(
                db, updated_by, f"UPDATE_PAYROLL_STATUS_{status.upper()}",
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error updating payroll status for record {record_id}: {str(e)}"
        )
...
async def create_manual_time_log(db: AsyncSession, user_id: int, start_time: datetime, end_time: datetime,
    ...
    db_log = TimeLog(user_id=user_id, start_time=start_time, end_time=end_time, is_manual=True,
                     break_duration_minutes=break_duration_minutes)
    db.add(db_log)
    await db.commit()
    await db.refresh(db_log)
...
async def update_leave_status(
    ...
    """Alias for update_leave_request_status to match test expectations"""
    return await update_leave_request_status(
        db, leave_id, status, admin_comment, approved_by
    )
