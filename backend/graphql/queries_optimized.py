...
import strawberry
from sqlalchemy import Time, desc, extract, func, select
...
class OptimizedQuery:
    ...
    async def me(self, info: strawberry.Info) -> types.User:
        ...
        if dataloaders:
            ...
            return types.User.from_pydantic(users[0]) if users[0] else None
    ...
    async def active_time_log(self, info: strawberry.Info) -> types.TimeLog | None:
        ...
        if active_log:
            return types.TimeLog.from_pydantic(active_log)
        return None
    ...
    async def roles(self, info: strawberry.Info) -> list[types.Role]:
        ...
        return [types.Role.from_pydantic(role) for role in roles_with_users if role]
    ...
    async def my_schedules(
        ...
        return [types.WorkSchedule.from_pydantic(s) for s in schedules[0]]
    ...
    async def work_schedules(
        ...
        return [types.WorkSchedule.from_pydantic(s) for s in result]
    ...
    async def time_logs(
        ...
        if query_start.tzinfo is not None:
            query_start = query_start.astimezone(datetime.UTC).replace(tzinfo=None)

        query_end = end_date
        if query_end.tzinfo is not None:
            query_end = query_end.astimezone(datetime.UTC).replace(tzinfo=None)
        ...
        if timelog_user_ids:
            user_loader = dataloaders["user_by_id"]
            {user.id: user for user in await user_loader.load_many(timelog_user_ids)}
        else:
            pass

        return [types.TimeLog.from_pydantic(l) for l in result]
    ...
    async def my_leave_requests(self, info: strawberry.Info) -> list[types.LeaveRequest]:
        ...
        return [types.LeaveRequest.from_pydantic(l) for l in user_leaves[0]]
    ...
    async def pending_leave_requests(self, info: strawberry.Info) -> list[types.LeaveRequest]:
        ...
        return [types.LeaveRequest.from_pydantic(r) for r in pending_leaves]
    ...
    async def all_leave_requests(self, info: strawberry.Info, status: str | None = None) -> list[types.LeaveRequest]:
        ...
        return [types.LeaveRequest.from_pydantic(r) for r in reqs]
    ...
    async def companies(self, info: strawberry.Info) -> list[types.Company]:
        ...
        return [types.Company.from_pydantic(c) for c in res]
    ...
    async def departments(self, info: strawberry.Info, company_id: int | None = None) -> list[types.Department]:
        ...
        return [types.Department.from_pydantic(d) for d in res]
    ...
    async def positions(self, info: strawberry.Info, department_id: int | None = None) -> list[types.Position]:
        ...
        return [types.Position.from_pydantic(p) for p in res]
    ...
    async def user_presences(
        ...
        for _user_id, data in presence_data.items():
            user = data["user"]
            ...
            results.append(types.UserPresence(
                user=types.User.from_pydantic(user),
    ...
    async def audit_logs(
        ...
        result = await db.execute(stmt)
        return [types.AuditLog.from_pydantic(log) for log in result.scalars().all()]
