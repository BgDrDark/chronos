...
class OptimizedDataLoader:
    ...
    def get_from_cache(self, key: KeyType) -> Any | None:
    ...
    async def batch_load(self, keys: list[KeyType]) -> list[Any | None]:
...
class UserDataLoader(OptimizedDataLoader):
    ...
    async def load_users_by_ids(self, keys: list[int]) -> list[User | None]:
...
class TimeLogDataLoader(OptimizedDataLoader):
    ...
    async def load_active_timelogs(self, user_ids: list[int]) -> list[TimeLog | None]:
...
async def load_roles_by_ids(session: AsyncSession, keys: list[int]) -> list[Role | None]:
