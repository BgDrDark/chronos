...
class HRMutation:
    ...
    async def create_employment_contract(
        self, input: inputs.EmploymentContractCreateInput, info: strawberry.Info
        ...
        if input.template_id:
            ...
            if template:
                ...
                clauses_result = await db.execute(
                    select(ContractTemplateClause).where(
                        ContractTemplateClause.template_id == input.template_id
                    )
                )
        ...
        if not final_clause_ids and template_clauses:
            final_clause_ids = (
                json.dumps(template_clauses) if template_clauses else None
            )
        ...
        contract = EmploymentContract(
            ...
            else (template.payment_day if template else 25),
            night_work_rate=input.night_work_rate
            if input.night_work_rate is not None
            else (template.night_work_rate if template else 0.5),
            overtime_rate=input.overtime_rate
            if input.overtime_rate is not None
            else (template.overtime_rate if template else 1.5),
            holiday_rate=input.holiday_rate
            if input.holiday_rate is not None
            else (template.holiday_rate if template else 2.0),
    ...
    async def generate_contract_number(
        self, company_id: int, info: strawberry.Info
    ) -> str:
        db = info.context["db"]
    ...
    async def get_contract_pdf_url(
        self, contract_id: int, info: strawberry.Info
    ) -> str:
            ...
            try:
                ...
                for idx, clause_id in enumerate(clause_id_list):
                    clause_assoc = ContractTemplateClause(
                        template_id=template.id, clause_id=clause_id, order_index=idx
                    )
                    db.add(clause_assoc)
            except Exception:
                pass
        ...
        default_sections = [
            {
                ...
                "content": "Работодателят предоставя на Работника работата на длъжността.",
                "order_index": 0,
                "is_required": True,
            },
            {
                "title": "Работно време и почивки",
                "content": f"Работникът изпълнява работата си в рамките на {work_hours_per_week} часа седмично.",
                "order_index": 1,
                "is_required": True,
            },
            {
                "title": "Права и задължения на работодателя",
                "content": "Работодателят е длъжен да осигури на Работника работата и необходимите условия.",
                "order_index": 2,
                "is_required": True,
            },
            {
                "title": "Права и задължения на работника",
                "content": "Работникът е длъжен да изпълнява работата лично и да спазва реда в предприятието.",
                "order_index": 3,
                "is_required": True,
            },
            {
                "title": "Заплащане",
                "content": "За извършената работа Работодателят заплаща трудовото възнаграждение.",
                "order_index": 4,
                "is_required": True,
            },
            {
                "title": "Конфиденциалност",
                "content": "Работникът се задължава да не разкрива на трети лица информация, станала му известна.",
                "order_index": 5,
                "is_required": False,
            },
        ]
    ...
    async def delete_contract_template(
        self, id: int, info: strawberry.Info | None = None
    ) -> bool:
        if not info:
            raise InvalidOperationException.info_required()
        db = info.context["db"]
    ...
    async def create_annex_template(
        ...
        template = AnnexTemplate(
            ...
            new_night_work_rate=Decimal(str(new_night_work_rate))
            if new_night_work_rate
            else None,
            new_overtime_rate=Decimal(str(new_overtime_rate))
            if new_overtime_rate
            else None,
            new_holiday_rate=Decimal(str(new_holiday_rate))
            if new_holiday_rate
            else None,
            is_active=True,
        )
        ...
        default_sections = [
            {
                "title": "Описание на промените",
                "content": "С настоящето споразумение се променят следните условия от трудовия договор:",
                "order_index": 0,
                "is_required": True,
            },
            {
                "title": "Основание",
                "content": "Настоящето споразумение се сключва на основание чл. 119, ал. 1 от Кодекса на труда.",
                "order_index": 1,
                "is_required": False,
            },
        ]
        ...
        return types.AnnexTemplate(
            ...
            new_base_salary=float(template.new_base_salary)
            if template.new_base_salary
            else None,
            new_work_hours_per_week=template.new_work_hours_per_week,
            new_night_work_rate=float(template.new_night_work_rate)
            if template.new_night_work_rate
            else None,
            new_overtime_rate=float(template.new_overtime_rate)
            if template.new_overtime_rate
            else None,
            new_holiday_rate=float(template.new_holiday_rate)
            if template.new_holiday_rate
            else None,
            is_active=template.is_active,
            created_at=template.created_at,
        )
    ...
    async def update_section_in_annex_template(
        ...
        from backend.database.models import AnnexTemplate, AnnexTemplateSection

        db_section = await db.get(AnnexTemplateSection, section_id)
        if not db_section:
            raise NotFoundException.resource("Section")

        template = await db.get(AnnexTemplate, db_section.template_id)
        if (
            template
    ...
    async def delete_section_from_annex_template(
        self, section_id: int, info: strawberry.Info | None = None
    ) -> bool:
        if not info:
            raise InvalidOperationException.info_required()
        db = info.context["db"]
    ...
    async def add_section_to_annex_template(
        ...
        return types.AnnexTemplateSection.from_pydantic(
            schemas.AnnexTemplateSection.model_validate(new_section)
        )
    ...
    @strawberry.mutation
    async def get_annex_pdf_url(self, annex_id: int, info: strawberry.Info) -> str:
        from backend.config import settings
    ...
    async def create_contract_annex(
        ...
        annex = ContractAnnex(
            ...
            work_hours_per_week=work_hours_per_week,
            night_work_rate=Decimal(str(night_work_rate))
            if night_work_rate is not None
            else None,
            overtime_rate=Decimal(str(overtime_rate))
    ...
    async def update_section_in_contract_template(
        ...
        from backend.database.models import ContractTemplate, ContractTemplateSection

        db_section = await db.get(ContractTemplateSection, section_id)
        if not db_section:
            raise NotFoundException.resource("Section")

        template = await db.get(ContractTemplate, db_section.template_id)
        if (
            template
            and template.company_id != current_user.company_id
            and current_user.role.name != "super_admin"
