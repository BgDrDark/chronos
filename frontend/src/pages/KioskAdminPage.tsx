...
import { 
  ...
  ListItemText, ListItemButton, ListItemIcon, Stack, Container
} from '@mui/material';
import {
...
const KioskSecuritySettings: React.FC = () => {
    ...
    const { data: rateData, loading: rateLoading, refetch: refetchRate } = useQuery(GET_VALIDATE_CODE_RATE_LIMIT, { fetchPolicy: 'network-only' });
    ...
    return (
        ...
        <Card sx={{ mb: 4, border: '1px solid #ff9800' }}>
            <CardContent>
                ...
                {rateMsg && (
                    <Typography color={rateMsg.includes('запазен') ? 'success.main' : 'error.main'} sx={{ mt: 1 }}>
                        {rateMsg}
                    </Typography>
                )}
            </CardContent>
